...
| :--- | :--- | :--- | :--- | :--- |
| 11 | FER_TEST | ✅ Saneado y Validado | Validación y saneamiento técnico de rama fer_test: navbar con control de roles, redirección raíz, endpoint /logout y corrección de '0 productos' en historial mediante RLS incremental en stock_outlet_items y fallback a inventory_logs. Se restauró init_v8.sql idéntica byte a byte a bbca07c y se aisló la política RLS exclusivamente en 20260902000000_fix_stock_outlet_items_rls.sql. tests/setup.test.ts actualizado para validar la migración incremental. Suite completa validada (Vitest: 87 passed, 0 failed, 1 skipped; Playwright: 1 passed, 0 failed). git diff --check y diff init_v8.sql limpios. Pendiente para sprint de integración: rebase/squash para sanear credenciales de commit histórico a866551 y ejecutar migración en Supabase Cloud. | N/A |
